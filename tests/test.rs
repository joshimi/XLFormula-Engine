use chrono::{format::ParseError, DateTime, Duration, FixedOffset};
...
fn evaluate_formula_number(s: &str) -> XlNum {
...
fn evaluate_formula_number_with_custom_function(
    s: &str,
    custom_function: Option<&impl Fn(String, Vec<XlNum>) -> types::Value>,
...
#[test]
#[cfg(feature = "f32")]
fn it_evaluate_negative_float_f32() {
...
#[test]
#[cfg(feature = "f64")]
fn it_evaluate_negative_float_f64() {
...
fn it_evaluate_functions_product() {
    assert_eq!(evaluate_formula_number("=PRODUCT(ABS(1),2*1, 3,4*1)"), 24.0,);
...
fn it_evaluate_references() {
    let data_function = |s: String| match s.as_str() {
        ...
        "Test" => types::Value::Iterator(vec![
            ...
            types::Value::Number(200.0),
            types::Value::Number(300.0),
        ]),
...
fn it_evaluate_multiple_iterators_and_scalars() {
    ...
    assert_eq!(evaluate_formula_number("=SUM({1,2,3},4,{5,6})"), 21.0,);
    assert_eq!(evaluate_formula_number("=SUM({1+1,2,3-3},4,{5,6*1})"), 19.0,);
    assert_eq!(
...
fn it_evaluate_custom_functions_() {
    ...
    assert_eq!(
        evaluate_formula_string_with_custom_function("=\"P\"&CountText()", Some(&custom_functions)),
        "P10"
    );
    assert_eq!(
...
fn it_evaluate_left_and_right_functions() {
    ...
    assert_eq!(evaluate_formula_string("=\"P\"&RIGHT(\"000\"&1,3)"), "P001",);
...
fn evaluate_invalid_reference_in_functions() {
    let data_function = |_s: String| types::Value::Error(types::Error::Value);
    // test with a non existing reference
    let formula =
        parse_formula::parse_string_to_formula("=AND(T, NO_REFERENCE)", None::<NoCustomFunction>);
    assert_eq!(
...
fn it_evaluates_blank_constructors() {
    let custom_functions = |s: String, _params: Vec<XlNum>| match s.as_str() {
